    ...
    row.update(
        {
            ...
            "accent_or_region": "general",
            "uploaded_date": "2025-01-01",
            "stored_date": "2025-02-01",
            "to_train": "yes",
            "tagged": "yes",
        }
    )
    ...
    with open(path, "w", newline="") as f:
        ...
        for i in range(n_si):
            ...
            w.writerow(
                _row(
                    source_id=src,
                    source_url=f"https://youtube.com/watch?v={src}",
                    language="sinhala",
                    title=f"Sinhala video {i % 5}",
                    genre="drama",
                    speaking_style="spontaneous",
                    language_formality="spoken",
                    code_switching="no_code",
                    duration_minutes=str(round(0.1 + (i % 10) * 0.05, 2)),
                    name_in_drive=f"rec{i:04d}.wav",
        ...
        for i in range(n_ta):
            ...
            w.writerow(
                _row(
                    ...
                    source_url=f"https://youtube.com/watch?v={src}",
                    language="tamil",
                    title=f"Tamil video {i % 4}",
                    genre="news",
                    speaking_style="read",
                    language_formality="spoken",
                    code_switching="no_code",
                    duration_minutes=str(round(0.15 + (i % 8) * 0.04, 2)),
                    name_in_drive=f"rec{n_si + i:04d}.wav",
...
def test_blocklist(tmp_path):
    ...
    cfg = _cfg(
        target_hours=10.0, min_segment_seconds=0.5, max_hours_per_channel=10.0, holdout_hours=0.01
    )
...
def test_report_lists_top_sources_with_titles(tmp_path):
    ...
    rows = [ln for ln in report.splitlines() if ln.startswith(("| 1 ", "| 2 "))]
    assert len(rows) == 2
...
def test_report_top_sources_capped_at_ten(tmp_path):
    ...
    table_rows = [
        ln
        for ln in report_path.read_text().splitlines()
...
def test_resolve_paths_disambiguates_by_genre_and_size_and_sets_aside_the_rest():
    lookup = build_path_lookup(
        [
            ...
            (
                "Tamil/drama/vidG/x_000.wav",
                100,
            ),  # other language: never a candidate for a Sinhala row
        ]
    )
    segs = [
    ...
    assert [s.relpath for s in resolved] == [
        "Sinhala/drama/vidA/x_000.wav",
        "Sinhala/drama/vidB/x_000.wav",
        "Sinhala/news/vidC/y_000.wav",
    ]
...
def test_clip_id_is_unique_even_when_sheet_names_collide():
    a = _clip("h_000.wav", 1)
    a.relpath = "Sinhala/drama/vidA/h_000.wav"
    b = _clip("h_000.wav", 1)
    b.relpath = "Sinhala/drama/vidB/h_000.wav"
    a_same = _clip("h_000.wav", 1)
    a_same.relpath = a.relpath
