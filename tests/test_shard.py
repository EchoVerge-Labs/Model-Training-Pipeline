...
from pipeline.shard import (  # noqa: E402
    concatenate_short_cuts, load_manifest_as_cuts, shard_cuts, shuffle_cuts, split_long_cuts,
)
...
def _make(tmp_path, clips):
    ...
    with open(manifest, "w", encoding="utf-8") as f:
        for cid, rel, seconds, channel in clips:
            ...
            sf.write(raw / rel, (np.random.randn(int(16000 * seconds)) * 0.1).astype("float32"), 16000)
            f.write(json.dumps({"id": cid, "filename": rel, "language": "sinhala", "genre": "drama",
                                "channel_id": channel}) + "\n")
    return manifest, raw


def test_manifest_to_cuts_to_shar_round_trip(tmp_path):
    manifest, raw = _make(tmp_path, [
    ...
    cuts = concatenate_short_cuts(cuts, min_sec=5.0, target_sec=15.0)   # 6 x 1s of src3 -> one 6s cut
    assert len(cuts) == 3
...
def test_repeated_clip_id_is_an_error(tmp_path):
    manifest, raw = _make(tmp_path, [("dup", "Sinhala/drama/vidA/a.wav", 1.0, "s"),
...
def test_split_windows_survive_sharding_with_the_right_audio(tmp_path):
    ...
    for c in back:
        ...
        assert n == pytest.approx(c.duration * 16000, abs=2)       # audio matches the window, not the whole file
        total_samples += n
